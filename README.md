...
**Analytics are already on, and there is nothing about them in this repo.**
Cloudflare Web Analytics runs on the site in "Automatic setup" mode, which means
Cloudflare injects the beacon at the edge on its way out. Nothing here loads it
and nothing here has to. Read it in the dashboard under Analytics, Web Analytics.
It is cookieless and stores nothing on the visitor, so there is no consent
banner to add.

Do not paste a beacon snippet or a token into the site to "switch it on". It is
already on, and a second beacon would count every page view twice. The only
reason to add one by hand would be moving the site off Cloudflare, and then the
automatic one stops with it.
